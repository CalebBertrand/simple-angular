...
export type ViewNode = ElementViewNode | ControlFlowViewNode | TextViewNode;

// intended to run on whatever's between the opening and closing angle brackets
function isValidOpenOrSelfClosingTag(str: string): false | ElementViewNode | ComponentViewNode {
  str = str.trim();
  ...
  return {
    type: componentBySelector.has(tagName) ? ViewNodeTypes.Component : ViewNodeTypes.Element,
    tagName,
    isSelfClosing,
    attributes,
    body: [],
...
function isValidAttribute(attr: string): false | ViewNodeAttribute {
  ...
  if (eqIndex === -1) {
    return isValidTagName(attr) ? { name: attr, value: true, isBound: false } : false;
  }
  ...
  while (index < template.length) {
    ...
    if (nextChar.match("\s")) {
      ...
      if (parent?.type === ViewNodeTypes.If) {
        ...
        while (nextIndex < template.length && template[nextIndex].match(/\s/)) {
          nextIndex++;
        }

        if (template.slice(nextIndex, nextIndex + 5) === "@else") {
          const elseOpeningBracket = template.indexOf("{", nextIndex + 5);
